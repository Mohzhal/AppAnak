...
export class HomePage implements OnInit {
  ...
  private initMap(): void {
    this.map = L.map('map').setView(this.childPosition, 13);
    ...
    if (navigator.geolocation) {
      navigator.geolocation.watchPosition(
        (position) => {
          ...
          if (this.map) {
            this.addMarker(this.currentLocation.lat, this.currentLocation.lng, 'Current Location');
            this.map.setView(
  ...
  private getParentLocation(): void {
    fetch('https://7543a7de-4ce1-43a6-b64e-9bcbe29a7c37-00-2p357v69b9bhz.worf.replit.dev/')
      .then((response) => response.json())
      .then((data) => {
        this.parentPosition = [data.latitude, data.longitude];
        ...
        this.addMarker(this.parentPosition[0], this.parentPosition[1], 'Parent Location');
      })
      .catch((error) => console.error('Error:', error));
  }
  ...
  private calculateDistance(): void {
    const latLng1 = L.latLng(this.childPosition);
    const latLng2 = L.latLng(this.parentPosition);
